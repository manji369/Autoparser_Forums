...
def grab_3_parents_by_ignoring_few(elem, row):
    parents_and_attrs = []
    parent_elem = elem
    i = 0
    while i < 3:
        ...
        if parent_elem.name not in text_parent_ignore_list:
            ...
            if parent_elem.name == 'a':
                parents_and_attrs = []
                i = 0
            parents_and_attrs = [(parent_elem.name, attrs)] + parents_and_attrs
            i += 1
    return parents_and_attrs

def find_3_parents(elem, row):
    parents_and_attrs = grab_3_parents_by_ignoring_few(elem, row)
    cur_elem = row
    prev_elem = row
...
def search_with_regex(attrs):
    for attr in attrs:
        reg_attr = regex_format_id(attrs[attr])
        if '[0-9]+' in reg_attr:
            return {attr: re.compile(reg_attr)}
    return attrs
...
def verify_parents_and_attrs(parents_and_attrs, row):
    ...
    if flag:
        ...
        if cur_elem:
            ...
            cur_elem = prev_elem.text
    return cur_elem


def regex_format_id(id_string):
    # return re.escape(re.search('[^0-9]+', id_string).group()) + re.sub('[0-9]+', '[0-9]+', re.search('[0-9]+', id_string).group())
    if '\\' in id_string:
        return id_string
    subs = re.subn(re.escape('[0-9]+'), '', id_string)
    if subs[1] == 1:
        subs = subs[0]
    elif subs[1] == 0:
        return id_string
    else:
        print 'Multiple chunks of numbers to replace in string'
        return False
    return re.escape(subs) + '[0-9]+'

def check_common_attrs_for_id(parent_1, attrs_1, parent_2, attrs_2):
    if parent_1 != parent_2:
        ...
        if 'id' in attrs_1 and 'id' in attrs_2:
            ...
            if attrs_1 == attrs_2:
                ...
                if id_1 == id_2:
                    id_1 = regex_format_id(id_1)
                    attrs_1 = {'id': id_1}
                    return attrs_1
    return False

def check_common_attrs_for_tag_attrs(parent_1, attrs_1, parent_2, attrs_2):
    if parent_1 != parent_2:
        return False
    else:
        if attrs_1 == attrs_2:
            ...
            if not attrs_1 and not attrs_2:
                return 'empty'
            elif len(attrs_1.keys()) == len(attrs_2.keys()):
                attrs_1_keys = attrs_1.keys()
                attrs_1_keys.sort()
                attrs_2_keys = attrs_2.keys()
                attrs_2_keys.sort()
                if attrs_1_keys == attrs_2_keys:
                    for key in attrs_1:
                        val_1 = attrs_1[key]
                        val_2 = attrs_2[key]
                        if val_1 != val_2:
                            val_1 = re.sub('[0-9]+', '[0-9]+', val_1)
                            val_2 = re.sub('[0-9]+', '[0-9]+', val_2)
                            if val_1 == val_2:
                                val_1 = regex_format_id(val_1)
                                if val_1:
                                    attrs_1 = {key: val_1}
                                else:
                                    attrs_1 = 'empty'
    ...
    if len(parents_and_attrs_only_1) == len(parents_and_attrs_only_2):
        ...
        while i < len(parents_and_attrs_only_1):
            ...
            while j < len(parents_and_attrs_only_2):
                ...
                regex_id = check_common_attrs_for_id(parent_1, dc(attrs_1), parent_2, dc(attrs_2))
                regex_attrs = check_common_attrs_for_tag_attrs(parent_1, attrs_1, parent_2, attrs_2)
                if regex_id:
                    if regex_id == 'empty':
                        regex_id = {}
                    parents_and_attrs_only.append((parent_1, regex_id))
                    j_start = j+1
                elif regex_attrs:
                    if regex_attrs == 'empty':
                        regex_attrs = {}
                    parents_and_attrs_only.append((parent_1, regex_attrs))
                    j_start = j+1
                    break
                j += 1
            i += 1
